...
def respond(
    ...
    Args:
        ...
        msg_originator (int):
                    The port of the node that created the original message (for a 0 TTL point-to-point message like a
                    PING, this will be the same as the forwarder)
        ttl (int):
                    Time-to-live; the number of hops remaining in the lifetime of this message until it should be
                    dropped. A 0 TTL message should not be forwarded.
        data (None or int):
                    The data in the message payload. For PINGs and PONGs, this will be None. For a PRIME message,
                    the data field will contain the prime number.

    Returns:
        Nothing
    """
    received_message_string = \
        str(msg_type) + str(msg_id) + str(msg_forwarder) + str(msg_originator) + str(ttl) + str(data)
    update_last_heard_from(msg_forwarder)

    if received_message_string not in RECEIVED_MESSAGES:
        RECEIVED_MESSAGES.add(received_message_string)
        if msg_type == PING:
            msg_to_send = {
                'msg_type': PONG,
                'ttl': 0,
            ...
            send_message_to(msg_forwarder, msg_to_send, False)
        elif msg_type == PRIME:
            if data > STATE['biggest_prime']:
            ...
            if ttl > 0:
                ...
                for peer in STATE['peers']:
                    if peer != msg_forwarder:
                        send_message_to(peer, msg_to_send, True)
...
def send_message_to(peer: int, message: dict, forwarded: bool):
    ...
    **YOU WILL NOT NEED TO MODIFY THIS FUNCTION.**
    ...
    if "msg_type" not in message or message["msg_type"] not in MESSAGE_TYPES:
    ...
    try:
        requests.post("http://localhost:%d/receive" % peer, json=message)
...
def proxy(node, method):
    ...
    """
    def stripped_headers(res):
        excluded_headers = ["content-encoding", "content-length", "transfer-encoding", "connection"]
        return [(name, value) for (name, value) in res.raw.headers.items() if name.lower() not in excluded_headers]
